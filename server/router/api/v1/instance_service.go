...
func convertInstanceStorageSettingFromStore(settingpb *storepb.InstanceStorageSetting) *v1pb.InstanceSetting_StorageSetting {
	...
	if settingpb.S3Config != nil {
		setting.S3Config = &v1pb.InstanceSetting_StorageSetting_S3Config{
			...
			InternalEndpoint: settingpb.S3Config.InternalEndpoint,
			PublicUrlBase:    settingpb.S3Config.PublicUrlBase,
			CacheControl:     settingpb.S3Config.CacheControl,
		}
	}
	return setting
}
...
func convertInstanceStorageSettingToStore(setting *v1pb.InstanceSetting_StorageSetting) *storepb.InstanceStorageSetting {
	...
	if setting.S3Config != nil {
		settingpb.S3Config = &storepb.StorageS3Config{
			AccessKeyId:      strings.TrimSpace(setting.S3Config.AccessKeyId),
			AccessKeySecret:  setting.S3Config.AccessKeySecret,
			Endpoint:         strings.TrimSpace(setting.S3Config.Endpoint),
			Region:           strings.TrimSpace(setting.S3Config.Region),
			Bucket:           strings.TrimSpace(setting.S3Config.Bucket),
			UsePathStyle:     setting.S3Config.UsePathStyle,
			InternalEndpoint: strings.TrimSpace(setting.S3Config.InternalEndpoint),
			PublicUrlBase:    strings.TrimSpace(setting.S3Config.PublicUrlBase),
			CacheControl:     strings.TrimSpace(setting.S3Config.CacheControl),
		}
	}
	return settingpb
}
